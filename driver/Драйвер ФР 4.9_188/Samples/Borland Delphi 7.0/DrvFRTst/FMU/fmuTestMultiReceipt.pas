...
begin
  ...
  case ReceiptType of
    0: begin
         Driver.StringforPrinting := Format('Продажа № %d', [Number]);
         Check(Driver.Sale);
       end;
    1: begin
         Driver.StringforPrinting := Format('Покупка № %d', [Number]);
         Check(Driver.Buy);
       end;
    2: begin
         Driver.StringforPrinting := Format('Возврат продажи № %d', [Number]);
         Check(Driver.ReturnSale);
       end;
    3: begin
         Driver.StringforPrinting := Format('Возврат покупки № %d', [Number]);
...
begin
  ...
  Driver.StringForPrinting := 'Чек #' + IntToStr(Number);
  Check(Driver.CheckSubTotal);
  Driver.Summ2 := 0;
  Driver.Summ3 := 0;
  Driver.Summ4 := 0;
