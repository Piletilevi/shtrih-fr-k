...
begin
  ...
  try
    ...
    Memo.Lines.Add('');
    AddSeparator;
    Memo.Lines.Add(' Запрос состояния купюроприемника:');
    AddSeparator;

    AddLineWidth('Режим опроса купюроприемника', CAModeTostr(Driver.CashAcceptorPollingMode), 29);
    AddLineWidth('Poll1', Driver.Poll1, 29);
    AddLineWidth('Poll2', Driver.poll2, 29);
    Memo.Lines.Add(' Описание: ' + Driver.PollDescription);
...
begin
  ...
  try
    ...
    Memo.Lines.Add('');
    AddSeparator;
    Memo.Lines.Add(' Запрос регистров купюроприемника:');
    AddSeparator;
    AddLineWidth('Номер набора регистров', IntToStr(Driver.RegisterNumber), 23);
    for i := 0 to 23 do
    begin
      Driver.BanknoteType := i;
      if Driver.ReadBanknoteCount <> 0 then
        Exit;
      AddLineWidth(Format('Кол-во купюр типа %.2d', [i]), Driver.BanknoteCount, 23);
    end;
    AddSeparator;
    // Прокручиваем Memo на начало
    Memo.SelStart := 0;
