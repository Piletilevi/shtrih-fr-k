...
uses
  ...
  StdCtrls,
  // This
  untPages, untDriver, TestManager, Buttons;
...
procedure TfmIBM.AddBitText(Value, ByteNumber, BitNumber: Integer;
...
procedure TfmIBM.AddBitText2(Value, BitNumber: Integer;
  const BitName: string);
var
  S: string;
  IsBitSet: Boolean;
...
procedure TfmIBM.AddStatusBytes;
begin
  AddSeparator;
...
procedure TfmIBM.AddStatusBits;
var
  Value: Integer;
begin
  // Status Byte 1
...
procedure TfmIBM.btnClearClick(Sender: TObject);
begin
  Memo.Lines.Clear;
end;

procedure TfmIBM.AddLine(const S1, S2: string);
var
  S: string;
begin
...
procedure TfmIBM.btnGetIBMStatusClick(Sender: TObject);
var
  S: string;
  Value: Integer;
  ValueName: string;
...
procedure TfmIBM.btnGetShortIBMStatusClick(Sender: TObject);
var
  ResultCode: Integer;
begin
  EnableButtons(False);
    ...
    begin
      ...
      begin
        ...
        try
          ...
          AddBitText2(Driver.IBMFlags, 0, 'Буфер печати ФР пуст');
          // Расшифровка байтов состояния
          AddStatusBits;
        finally
          Lines.EndUpdate;
